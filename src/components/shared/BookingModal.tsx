...
export const BookingModal: React.FC<BookingModalProps> = ({ isOpen, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
      <div className="relative w-full max-w-2xl bg-[#071228] border border-[#F2B84B]/30 rounded-2xl shadow-2xl overflow-hidden text-white">
        ...
        {submitted ? (
          <div className="p-10 text-center space-y-6">
            <div className="w-16 h-16 bg-[#1FA971]/20 border border-[#1FA971] rounded-full flex items-center justify-center mx-auto text-[#34D399]">
              <CheckCircle2 className="w-10 h-10" />
            </div>
            <div>
              <h3 className="text-2xl font-bold text-white mb-2">Consultation Request Received</h3>
              <p className="text-sm text-slate-300 max-w-md mx-auto">
                Thank you, <span className="text-[#F2B84B] font-semibold">{formData.fullName}</span>. Our advisory team will reach out to <span className="text-white font-medium">{formData.email}</span> within 24 hours to schedule your strategic financial assessment.
              </p>
            </div>
            <div className="p-4 bg-white/5 rounded-xl text-xs text-slate-400 max-w-sm mx-auto flex items-center gap-2 text-left border border-white/10">
          ...
          <div className="p-6 sm:p-8">
            <div className="flex items-center justify-between mb-6 pb-4 border-b border-white/10">
              <div>
                ...
                <h2 className="text-xl sm:text-2xl font-bold text-white">Book Your Consultation</h2>
              </div>
              <AhaduLogo variant="white" size="sm" showSubtitle={false} />
            </div>
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3.5 py-2.5 bg-white/5 border border-white/15 rounded-lg text-sm text-white placeholder-slate-500 focus:outline-none focus:border-[#1FA971] transition-colors"
                  />
                </div>
                <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-medium text-slate-300 mb-1">Primary Service Area</label>
                  <select
                    value={formData.serviceInterest}
                    onChange={(e) => setFormData({ ...formData, serviceInterest: e.target.value })}
                    className="w-full px-3.5 py-2.5 bg-[#0B1B3A] border border-white/15 rounded-lg text-sm text-white focus:outline-none focus:border-[#1FA971]"
                  >
              ...
              <div>
                <label className="block text-xs font-medium text-slate-300 mb-1">Brief Overview of Financial Goals or Challenges</label>
                <textarea
                  rows={3}
